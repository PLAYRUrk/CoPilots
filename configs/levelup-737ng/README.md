...
- **Датарефы (onchange)** — позиции переключателей, MCP-крутилки, баро, EFIS, свет, яркость, радио, отказы: секции `[TRIGGERS]` + `[RADIO]` + `[SLOW]` исходного SmartCopilot-конфига.
- **Датарефы (continuous, только от физмастера)** — стояночный тормоз, триммеры (тангаж/крен/курс).
- **Команды (688, с ретрансляцией удержания)** — все кнопки и тумблеры, управляемые Lua-логикой: FMC обоих CDU, пожарная панель, тесты (stall/overspeed/fire), генераторы, packs, стартеры, spring-переключатели.
